module QiniuUtility
  class ObjectStorage
    ...
    def list(bucket, limit: 100, marker: nil)
      list_policy = Qiniu::Storage::ListPolicy.new(bucket, limit)
        .tap { |policy| policy.marker = marker if marker.present? }
      code, result, _response_headers, _s, d = Qiniu::Storage.list(list_policy)
      QiniuUtility.logger.info "#{self.class.name} list #{bucket} resp(#{code}): #{result['error'] || { items_count: result['items'].count }.to_json}; #{d.to_json}"
      result
    end
